...
void string_concat(char *string1, char *string2, unsigned int no_of_chars)
{
    if (string1 == NULL ||
        string2 == NULL ||
        no_of_chars == 0)
    ...
    for (unsigned int i = 0; i < (no_of_chars - 1); i++)
    {
        string1[i + string1_length] = string2[i];

        if (string2[i] == '\0')
            return;
    }

    string1[string1_length + no_of_chars - 1] = '\0';
